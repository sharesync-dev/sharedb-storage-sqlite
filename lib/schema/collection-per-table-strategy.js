...
CollectionPerTableStrategy.prototype.writeInventoryFromMeta = function(db, inventoryMeta) {
  ...
  // Clear existing inventory and repopulate from meta
  promises.push(self.runAsync(db, 'DELETE FROM sharedb_inventory'));
  ...
  for (var collection in collections) {
    for (var docId in collections[collection]) {
      ...
      promises.push(self.runAsync(db,
        'INSERT INTO sharedb_inventory (collection, doc_id, version_num, version_str, has_pending, updated_at) ' +
        'VALUES (?, ?, ?, ?, ?, ?)',

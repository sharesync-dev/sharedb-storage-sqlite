...
CollectionPerTableStrategy.prototype.updateProjections = function(db, collection, newRecord, oldRecord) {
  ...
  for (var i = 0; i < projections.length; i++) {
    ...
    var arrayValue = this.getValueAtPath(newRecord.payload, projection.arrayPath);
...
CollectionPerTableStrategy.prototype.readAllRecords = function(db, type, collection, callback) {
  ...
  if (type === 'meta') {
    ...
    promise = this.getAllAsync(db, "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sharedb_%' AND name NOT LIKE 'sqlite_%' AND name NOT IN ('term_user_tag')")

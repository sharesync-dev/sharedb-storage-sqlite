  ...
  return this.runAsync(db, createTableSQL).then(function() {
    ...
    if (config.indexes && config.indexes.length > 0) {
      ...
      for (var i = 0; i < config.indexes.length; i++) {
        ...
        // Use raw SQL for json_extract index expression (squilt doesn't support expression indexes yet)
        var indexSQL = 'CREATE INDEX IF NOT EXISTS ' + indexName + ' ON ' + tableName +

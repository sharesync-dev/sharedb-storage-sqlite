...
CollectionPerTableStrategy.prototype.writeRecords = function(db, recordsByType, callback) {
  ...
  // Debug logging removed - compound key handling is now working correctly
...
CollectionPerTableStrategy.prototype.readRecord = function(db, type, collection, id, callback) {
  ...
  if (type === 'meta') {
    ...
    // We need to reconstruct the full inventory from our sharedb_inventory table
    if (id === 'inventory') {
      promise = this.readInventory(db).then(function(inventoryRecord) {
        // readInventory already returns { id: 'inventory', payload: { collections: ... } }
        // Return the full record - SqliteStorage will extract the payload
    ...
    var tableName = this.getTableName(collection);
    console.log('[CollectionPerTableStrategy.readRecord] Reading from table:', tableName, 'id:', id);
    // Use simple ID for querying
    promise = this.getFirstAsync(db, 'SELECT data FROM ' + tableName + ' WHERE id = ?', [id])
      .then(function(row) {
        console.log('[CollectionPerTableStrategy.readRecord] Query result:', row ? 'found' : 'not found');
        if (!row) return null;
        var record = JSON.parse(row.data);
        console.log('[CollectionPerTableStrategy.readRecord] Parsed record has keys:', Object.keys(record));
        console.log('[CollectionPerTableStrategy.readRecord] Has payload:', !!record.payload);

        // IMPORTANT: The record structure in the database is:
        // { id: "collection/docId", payload: { ... ShareDB document ... } }
        // We need to return this full structure to SqliteStorage
        console.log('[CollectionPerTableStrategy.readRecord] Returning record with id:', record.id);
        ...
        console.log('[CollectionPerTableStrategy.readRecord] About to return record, checking structure:', {
          hasRecord: !!record,
          hasPayload: !!(record && record.payload),
          payloadType: record && record.payload ? typeof record.payload : 'N/A'
        });

        // Log the full record for debugging
        console.log('[CollectionPerTableStrategy.readRecord] Full record being returned:',

...
CollectionPerTableStrategy.prototype.writeCollectionRecord = function(db, collection, record) {
  ...
  // ShareDB sends compound IDs, but we need to store simple IDs in inventory
  var simpleDocId = record.id;
  if (record.id && record.id.indexOf('/') !== -1) {
    // Split on the first '/' to handle nested collections
    var parts = record.id.split('/');
    if (parts[0] === collection) {
      // Remove the collection prefix
      simpleDocId = parts.slice(1).join('/');
    }
  }
  ...
  return this.getFirstAsync(db, 'SELECT data FROM ' + tableName + ' WHERE id = ?', [record.id])
    .then(function(oldRow) {
      ...
      // Write the record (keep compound ID for backward compatibility)
      return self.runAsync(db,
...
CollectionPerTableStrategy.prototype.updateInventoryForRecord = function(db, collection, docId, version, hasPending) {
  // Guard against compound IDs in inventory (collection/docId format)
  // Note: We allow slashes in IDs for nested paths, but not when it starts with collection name
  if (docId && docId.indexOf('/') !== -1) {
    var firstSlashIndex = docId.indexOf('/');
    var prefix = docId.substring(0, firstSlashIndex);
    if (prefix === collection) {
      throw new Error('Inventory docId appears to be a compound ID. Got: "' + docId + '". ' +
        'Collection prefix "' + collection + '/" should be stripped before storing in inventory. ' +
        'Use the simple document ID instead.');
    }
  }
...
CollectionPerTableStrategy.prototype.readRecord = function(db, type, collection, id, callback) {
  ...
  if (type === 'meta') {
    ...
    var tableName = this.getTableName(collection);
    // Construct the compound ID for querying (collection/id format)
    var compoundId = collection + '/' + id;
    promise = this.getFirstAsync(db, 'SELECT data FROM ' + tableName + ' WHERE id = ?', [compoundId])
      .then(function(row) {
        if (!row) return null;
...
CollectionPerTableStrategy.prototype.deleteRecord = function(db, type, collection, id, callback) {
  ...
  if (type === 'meta') {
    ...
    var tableName = this.getTableName(collection);
    // Construct the compound ID for deletion (collection/id format)
    var compoundId = collection + '/' + id;
    promise = this.runAsync(db, 'DELETE FROM ' + tableName + ' WHERE id = ?', [compoundId])
      .then(function() {
...
CollectionPerTableStrategy.prototype.updateInventoryItem = function(db, collection, docId, version, operation, callback) {
  ...
  if (docId && docId.indexOf('/') !== -1) {
    ...
    var prefix = docId.substring(0, firstSlashIndex);
    if (prefix === collection) {
      var error = new Error('Inventory docId appears to be a compound ID. Got: "' + docId + '". ' +
        'Collection prefix "' + collection + '/" should be stripped before storing in inventory. ' +
        'Use the simple document ID instead.');
      if (callback) {
        callback(error);

...
class CollectionPerTableStrategy extends BaseSchemaStrategy_1.BaseSchemaStrategy {
    ...
    async createProjectionTables(db, collection) {
        ...
        for (const projection of projections) {
            ...
            for (const column of projection.primaryKey) {
                ...
                await this.runAsync(db, 'CREATE INDEX IF NOT EXISTS ' + indexName + ' ON ' + projectionTableName + '(' + column + ')');
            }
    ...
    async writeRecords(db, recordsByType, callback) {
        try {
            ...
            if (recordsByType.docs) {
                ...
                if (typeof recordsByType.docs === 'object' && !Array.isArray(recordsByType.docs) && !recordsByType.docs.id) {
                    // docs is already a dictionary of collections
                    recordsByCollection = recordsByType.docs;
                }
                else {
                ...
                for (const [collection, records] of Object.entries(recordsByCollection)) {
                    ...
                    for (const record of records) {
                        // Write record
                        await this.runAsync(db, 'INSERT OR REPLACE INTO ' + tableName + ' (id, collection, data) VALUES (?, ?, ?)', [record.id, collection, JSON.stringify(record)]);
                        // Update projections
                        ...
                        const version = record.version || record.payload?.v || 1;
                        const hasPending = (record.payload?.pendingOps || record.payload?.inflightOp) ? 1 : 0;
                        if (typeof version === 'string') {
                            await this.runAsync(db, 'INSERT OR REPLACE INTO sharedb_inventory (collection, doc_id, version_num, version_str, has_pending, updated_at) VALUES (?, ?, NULL, ?, ?, ?)', [collection, record.id, version, hasPending, Date.now()]);
                        }
                        else {
                            await this.runAsync(db, 'INSERT OR REPLACE INTO sharedb_inventory (collection, doc_id, version_num, version_str, has_pending, updated_at) VALUES (?, ?, ?, NULL, ?, ?)', [collection, record.id, version, hasPending, Date.now()]);
                        }
            ...
            if (recordsByType.meta) {
                ...
                for (const metaRecord of metaRecords) {
                    await this.runAsync(db, 'INSERT OR REPLACE INTO sharedb_meta (id, data) VALUES (?, ?)', [metaRecord.id, JSON.stringify(metaRecord.payload)]);
                }
            }
            callback?.();
        }
        ...
        try {
            if (type === 'meta') {
                ...
                if (!collection || collection === 'docs') {
                    const inventoryRow = await db.getFirstAsync('SELECT collection FROM sharedb_inventory WHERE doc_id = ?', [id]);
                    if (!inventoryRow) {
                        callback?.(null, null);
                        return null;
                    }
    ...
    async readInventory(db, callback) {
        try {
            const rows = await db.getAllAsync('SELECT collection, doc_id, version_num, version_str, has_pending FROM sharedb_inventory ORDER BY collection, doc_id');
    ...
    async upsertInventoryItem(db, collection, docId, version, callback) {
        try {
            const now = Date.now();
            // Check for version type consistency
            const existing = await db.getFirstAsync('SELECT version_num, version_str FROM sharedb_inventory WHERE collection = ? AND doc_id = ?', [collection, docId]);
            if (existing) {
                const existingIsString = (existing.version_str !== null);
                const newIsString = (typeof version === 'string');
                if (existingIsString !== newIsString) {
                    throw new Error(`Version type mismatch: Cannot store ${collection}/${docId} with ${typeof version} version ${version} when existing version is ${existingIsString ? 'string' : 'number'}`);
                }
            }
            // Insert or update with appropriate version column
            if (typeof version === 'string') {
                await this.runAsync(db, 'INSERT OR REPLACE INTO sharedb_inventory (collection, doc_id, version_num, version_str, has_pending, updated_at) VALUES (?, ?, NULL, ?, 0, ?)', [collection, docId, version, now]);
            }
            else {
                await this.runAsync(db, 'INSERT OR REPLACE INTO sharedb_inventory (collection, doc_id, version_num, version_str, has_pending, updated_at) VALUES (?, ?, ?, NULL, 0, ?)', [collection, docId, version, now]);
            }
    ...
    async deleteInventoryItem(db, collection, docId, callback) {
        try {
            await this.runAsync(db, 'DELETE FROM sharedb_inventory WHERE collection = ? AND doc_id = ?', [collection, docId]);
            callback?.();
        }
        catch (error) {
            callback?.(error);
    ...
    async deleteAllTables(db, callback) {
        try {
            ...
            const tables = await db.getAllAsync("SELECT name FROM sqlite_master WHERE type='table' AND name NOT IN ('sharedb_meta', 'sharedb_inventory')");
     ...
     */
    async ensureCollectionTable(db, collection) {

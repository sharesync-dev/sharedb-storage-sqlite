  ...
  return this.runAsync(db,
    ...
    if (config.indexes && config.indexes.length > 0) {
      ...
      for (var i = 0; i < config.indexes.length; i++) {
        ...
        var baseIndexName = 'idx_' + tableName.split('.').pop() + '_' + sanitizedField;
        var indexName = self.attachmentAlias
          ? self.attachmentAlias + '.' + baseIndexName
          : baseIndexName;
        ...
        var indexQuery = CREATE_INDEX(indexName)
          .on(onTable, FN('json_extract', COLUMN('data'), new StringLiteral('$.' + field)))
          .ifNotExists();

...
AttachedCollectionPerTableStrategy.prototype.getTableName = function(collection) {
  ...
  if (collection === '__meta__') {
    baseTableName = 'sharedb_meta';
  } else if (collection === '__inventory__') {
...
AttachedCollectionPerTableStrategy.prototype.initializeSchema = function(db, callback) {
  var self = this;
  var metaTable = self.getTableName('__meta__');
...
AttachedCollectionPerTableStrategy.prototype.validateSchema = function(db, callback) {
  ...
  var promise = Promise.all([
    db.getFirstAsync(
      "SELECT name FROM " + sqliteMaster + " WHERE type='table' AND name='sharedb_meta'"
    ),
    db.getFirstAsync(
...
AttachedCollectionPerTableStrategy.prototype.deleteAllTables = function(db, callback) {
  ...
  var promise = db.getAllAsync(
    "SELECT name FROM " + sqliteMaster + " WHERE type='table' AND (name LIKE 'sharedb_%' OR name LIKE 'collection_%' OR name LIKE 'projection_%')"

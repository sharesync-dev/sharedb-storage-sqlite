...
AttachedSqliteAdapter.prototype.connect = function() {
  ...
  // If already attached, return immediately (idempotent)
  if (adapter.attached) {
    adapter.debug && console.log('[AttachedSqliteAdapter] Already attached, skipping connection');
...
AttachedSqliteAdapter.prototype.disconnect = function() {
  ...
  // Refuse new work immediately, before the underlying close settles, so
  // operations racing the disconnect fail fast instead of hitting a
  // half-closed handle (and then trying to re-ATTACH against it).
  adapter.closed = true;
...
/**
 * Rejection helper for operations attempted after disconnect()
 * @private
 */
AttachedSqliteAdapter.prototype._rejectClosed = function() {
  return Promise.reject(new Error('AttachedSqliteAdapter is disconnected'));
};
...
AttachedSqliteAdapter.prototype.ensureAttached = function() {
  ...
  // After disconnect(), fail fast: never probe or re-attach a closed connection
  if (adapter.closed) {
    return adapter._rejectClosed();
  }
  ...
  return adapter.wrappedAdapter.getAllAsync("PRAGMA database_list").then(function(databases) {
    ...
    // If the adapter was disconnected (possibly mid-check), the error is
    // expected -- propagate it rather than attempting to re-attach.
    if (adapter.closed) {
      return adapter._rejectClosed();
    }

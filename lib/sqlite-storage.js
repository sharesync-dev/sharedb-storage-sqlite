...
SqliteStorage.prototype.readRecord = function(storeName, id, callback) {
  ...
  this.schemaStrategy.readRecord(this.adapter, type, collection, docId, function(error, record) {
    if (error) {
      ...
      if (record) {
        console.log('[SqliteStorage.readRecord] Got record for', id, '- has payload:', !!record.payload);
        if (!record.payload) {
          console.log('[SqliteStorage.readRecord] No payload! Record structure:', JSON.stringify(record, null, 2));
        } else {
          console.log('[SqliteStorage.readRecord] Payload type:', typeof record.payload);
        }
      } else {
        console.log('[SqliteStorage.readRecord] No record returned for', id);
      }
